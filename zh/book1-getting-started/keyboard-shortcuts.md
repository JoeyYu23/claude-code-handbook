...
配置后，`Option+B`、`Option+F`、`Option+T` 等快捷键就能正常工作了。
...
| `Ctrl+F` | 终止所有后台代理（在 3 秒内连按两次以确认） |

### 模型与思考

| 快捷键 | 功能 |
|---|---|
| `Option+P`（Mac）/ `Alt+P`（Win/Linux） | 不清空提示直接切换模型 |
| `Option+T`（Mac）/ `Alt+T`（Win/Linux） | 切换扩展思考模式开/关（需先运行 `/terminal-setup`） |
