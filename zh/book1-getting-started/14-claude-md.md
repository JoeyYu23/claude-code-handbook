...
对于大型项目，可以把指令拆分到 `.claude/rules/` 目录下的多个 Markdown 文件中，而不是全部塞进一个 CLAUDE.md。每个文件覆盖一个主题（如 `testing.md`、`api-design.md`），支持递归发现子目录。

没有 frontmatter 的 rules 文件在 session 启动时与 CLAUDE.md 一起加载。通过 `paths:` frontmatter 可以限定规则只在 Claude 操作匹配路径的文件时生效：
...
所有 API handler 必须使用 Zod 做输入验证。
错误响应统一使用标准格式。
```

`paths` 支持 glob 模式（`**/*.ts`、`src/components/*.tsx` 等），可以指定多个模式。

Rules 和 CLAUDE.md 是互补关系，不是替代——适合不同子目录有不同规范的场景。个人级 rules 放在 `~/.claude/rules/`，对所有项目生效，但优先级低于项目级 rules。
