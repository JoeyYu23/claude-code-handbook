...
**查看当前 session 的文件访问权限：**

```text
/permissions
```

此命令显示 Claude 在当前 session 中被授权访问的文件、目录和工具。在处理包含凭据或私密数据的代码库之前，使用此命令验证 Claude 无法访问敏感文件。

**用 .claudeignore 排除文件：**

在项目根目录创建 `.claudeignore` 文件，阻止 Claude 读取指定文件或目录：
...
`.claudeignore` 中列出的文件对 Claude 的文件读取和代码库探索完全不可见——即使你要求 Claude "探索项目"，它也不会读取这些文件。这与 `.gitignore`（只影响 git）不同，为本地敏感文件提供额外的安全保护层。
...
| 医疗数据（HIPAA） | 需要 Anthropic 的 BAA（商业伙伴协议）；Anthropic 持有 SOC 2 Type 2 和 ISO 27001 认证，详见 trust.anthropic.com |
...
1. **不要**公开披露
2. 通过 Anthropic 的 HackerOne 漏洞披露计划报告：[hackerone.com/anthropic-vdp](https://hackerone.com/anthropic-vdp)
3. 提供详细的复现步骤
4. 等待 Anthropic 确认并修复后再公开披露
...
你已完成《Claude Code 手册》全部内容。每章都会随 Claude Code 的更新持续维护。

- [Star 这个项目](https://github.com/JoeyYu23/claude-code-handbook) 关注更新
- [提交 Issue](https://github.com/JoeyYu23/claude-code-handbook/issues) 反馈错误或建议
- [查看附录](/zh/book2-advanced/agent-reference) 获取速查资料
