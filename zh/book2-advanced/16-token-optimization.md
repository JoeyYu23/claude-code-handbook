...
| Cache read tokens | 命中 prompt cache 的 input tokens | 约 10% input 价格 |
| Thinking tokens | Extended thinking 模式下的内部推理 | 与 output 相同 |

### 一个典型 session 的成本构成

```
典型编码 session（1小时）：
- Input tokens:  ~50,000  → $0.15  (Claude Sonnet 4)
...
| 模型 | 智力水平 | 速度 | 适用场景 |
|------|---------|------|---------|
| Claude Opus 4 | 最强 | 最慢 | 复杂架构设计、多文件调试、研究分析 |
| Claude Sonnet 4 | 强 | 快 | 日常编码、功能实现、代码审查（默认推荐） |
| Claude Haiku 3.5 | 良 | 极快 | 简单查询、格式化、低复杂度任务 |

### 实际使用建议

```bash
# 切换模型（在 session 中）
/model claude-sonnet-4-5    # 日常使用
/model claude-opus-4        # 复杂问题
/model claude-haiku-3-5     # 批量简单任务
```
...
```bash
# 用不同模型处理同一任务，比较成本和质量
claude --model claude-haiku-3-5 --print "解释这个函数" < func.ts
claude --model claude-sonnet-4-5 --print "解释这个函数" < func.ts
```
...
下表基于 Claude Sonnet 4 的实际使用统计（仅供参考，价格随 Anthropic 定价调整）：
