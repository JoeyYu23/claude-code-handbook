...
skills:                           # 预加载的 skills
  - api-conventions
  - error-handling
initialPrompt: "分析认证模块..."  # 启动时自动发给 agent 的初始提示词
mcpServers:                       # 此 agent 专属的 MCP 服务器（内联配置）
  github:
    type: http
    url: https://api.githubcopilot.com/mcp/
hooks:                            # 此 agent 执行生命周期内的 hooks
  PreToolUse:
    - matcher: "Bash"
      hooks:
        - type: command
...
| 字段 | 作用 |
|---|---|
| `name` | 引用此 agent 的唯一标识符 |
| `description` | Claude 据此决定何时委托给这个 agent |
| `tools` | 工具白名单，省略则继承主对话的全部工具 |
| `disallowedTools` | 从继承或指定的工具集中移除的工具黑名单 |
| `model` | 此 agent 使用的模型：`haiku`/`sonnet`/`opus`/`inherit` 或完整模型 ID |
| `effort` | 此 agent 模型的算力级别：`low`/`medium`/`high`/`max`/`auto` |
| `permissionMode` | agent 处理权限提示的方式：`default`/`acceptEdits`/`dontAsk`/`bypassPermissions`/`plan` |
| `maxTurns` | agent 在停止前可执行的最大轮数 |
| `memory` | 持久记忆范围：`user`/`project`/`local` |
| `background` | 设为 `true` 则始终作为后台任务运行 |
| `isolation` | 设为 `worktree` 则在临时 git worktree 中运行 |
| `skills` | 启动时预加载到 agent 上下文中的 skill 名称列表 |
| `effort` | agent 运行时的模型算力级别 |
| `initialPrompt` | 启动时自动发给 agent 的第一条用户消息 |
| `mcpServers` | 仅此 agent 可用的 MCP server 配置（与 `.mcp.json` 格式相同） |
| `hooks` | 限定在此 agent 执行生命周期内的 hooks |
...
Agent Teams 是一个实验性功能，允许多个 Claude 实例真正并行运行，每个实例都在自己的终端窗格中工作。与标准 sub-agent 模型（agent 被生成后返回结果）不同，teammates 是并排运行的完整 Claude session，通过共享状态进行协调。
...
**`auto`** — Claude 根据任务复杂度自行决定是内联工作还是生成 teammates。

**`in-process`** — Teammates 作为同一进程内的线程运行。开销更低，适合耦合较紧的任务。

**`tmux`** — 每个 teammate 在独立的 tmux 窗格中运行，在终端中可见。最适合需要监控进度的长时间并行任务。

以 tmux teammate 模式启动 session：

```bash
CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1 claude --teammate-mode tmux
```

Agent Teams 与内置 `/batch` skill 配合使用，可以自动将工作分发到多个 worktrees 并并行运行 agents。详细的并行编排模式见第六章。
