...
**示例 Hook 配置（在 `.claude/settings.json` 中配置）：**
...
Hooks 配置位于 `settings.json` 的 `"hooks"` 字段中，使用官方事件名作为顶级 key：`PreToolUse`、`PostToolUse`、`UserPromptSubmit`、`Stop`、`Notification`、`SubagentStart`、`SubagentStop` 等。每个事件下是一个数组，每个元素包含 `matcher`（匹配条件）和 `hooks`（要执行的 hook 列表）。

当 Claude 完成任务（Stop 事件）时，此 Hook 运行验证脚本。脚本返回退出码 0 则继续，返回 2 则操作被阻止并显示错误信息。
...
- **permissions.allow：** Claude 可以无需确认直接使用的工具和命令（白名单）
- **permissions.deny：** Claude 绝对不能使用的工具和命令（黑名单）
- **permissions.defaultMode：** 默认权限模式（如 `bypassPermissions`）
- **hooks：** 在特定事件时自动触发的脚本（见第二层）
...
有了这套配置，Claude 可以自由运行测试和构建命令，但绝对不能运行 `rm -rf` 或 `sudo`。`allow` 列表中的工具无需确认即可使用，`deny` 列表中的工具被完全禁止。未列出的工具则根据 `defaultMode` 决定行为。
...
### 第二步：创建 Settings 与 Hook 配置

Hooks 不使用独立的配置文件，而是配置在 `settings.json` 的 `"hooks"` 字段中：
...
Settings 已经在第二步创建的 `settings.json` 中配置完成。`permissions` 字段控制工具访问权限，`hooks` 字段控制自动化触发器。两者统一在同一个文件中管理，无需单独创建。

如需添加更多限制，直接编辑 `acme-app/.claude/settings.json` 即可。例如，添加更多 deny 规则：
...
6. 验证 Claude 无法运行 `rm -rf`（被 `permissions.deny` 阻止）
